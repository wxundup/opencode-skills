...
Explains what a change breaks somewhere else, beyond the diff.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the PR and its commits with `gh` or `git log` when available.
...
6. For a big or wide change, fan out parallel review subagents (Task tool) on the same question and merge the answers. Different models catch different real bugs.
